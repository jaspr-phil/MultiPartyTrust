...
public class ContextAggModel2 extends Model {
    NodeNamer nodeNamer;
    Map<String, ContextAggModel2.Aggregate> aggs;

    public ContextAggModel2(NodeNamer nodeNamer) {
    ...
    public void learn(List<Graph<String, String>> graphs, List<Double> values) {
        ...
        do {
            ...
            double value = (Double)valueIter.next() / (double)graphEdges.size();
            Iterator var9 = graphEdges.iterator();

            while(var9.hasNext()) {
                String edge = (String)var9.next();
                if (!this.aggs.containsKey(edge)) {
                    this.aggs.put(edge, new ContextAggModel2.Aggregate(value, 1));
                } else {
                    ((ContextAggModel2.Aggregate)this.aggs.get(edge)).addObservation(value);
                }
            }
    ...
    public double predict(Graph<String, String> graph) {
        if (this.aggs.isEmpty()) {
            ...
            while(var6.hasNext()) {
                ...
                if (this.aggs.containsKey(edge)) {
                    sum += ((ContextAggModel2.Aggregate)this.aggs.get(edge)).mean();
                    ++count;
                }
            }
