...
public class AggModel extends Model {
    NodeNamer nodeNamer;
    Map<String, AggModel.Aggregate> aggs;

    public AggModel(NodeNamer nodeNamer) {
        ...
        do {
            ...
            while(var9.hasNext()) {
                ...
                if (!this.aggs.containsKey(node)) {
                    this.aggs.put(node, new AggModel.Aggregate(value, 1));
                } else {
                    ((AggModel.Aggregate)this.aggs.get(node)).addObservation(value);
                }
            }
    ...
    public double predict(Graph<String, String> graph) {
        if (this.aggs.isEmpty()) {
            ...
            while(var6.hasNext()) {
                ...
                if (this.aggs.containsKey(node)) {
                    sum += ((AggModel.Aggregate)this.aggs.get(node)).mean();
                    ++count;
                }
            }
